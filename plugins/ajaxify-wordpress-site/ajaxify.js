	...
	$(function(){
		...
		$.fn.ajaxify = function(){
			...
			$this.find('a:internal:not(.no-ajaxy,[href^="#"],[href*="wp-login"],[href*="wp-admin"])').on('click', function(event){
				...
				if($(this).parent().hasClass('current-menu-item').toString()=='false'){
		...
		$(window).bind('statechange',function(){
			...
			if ( '' != aws_data['transition'] ) {
				$('div#primary').animate({opacity:0},300);
			}
			if( '' != aws_data['loader'] ) {
				$content
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR){
					...
					var $menu_list = $data.find('ul#primary-menu');
					var $menu_list2 = $data.find('aside#secondary');
					var $menu_list3 = $data.find('div#lang');
					...
					$content.html(contentHtml)
							...
							.css('text-align', '')
							;
					$('div#primary').animate({opacity: 1, visibility: "visible"}, 300);
					...
					// runPage();
					// checkNextPrevLinks();
					// if(isMobile===true) closeMenu();
					
					//Append new menu HTML to provided classs
					// CUSTOMIZED
					$('ul#primary-menu').html($menu_list.html());
					$('aside#secondary').html($menu_list2.html());
					$('div#lang').html($menu_list3.html());
